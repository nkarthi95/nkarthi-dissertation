...
size}\label{field-strength-dependence-on-domain-size}
...
We characterize the microstructure changes observed through calculating
a characteristic length scale, called the average domain size. The
average domain size is calculated using the first moment of the 3D
averaged structure factor. Past work has shown that this method
correctly reproduces the scaling expected in phase separating systems.
The structure factor is calculated from the fourier transform of the
fluctuations in the order parameter. \cite{kendon_inertial_2001}

\begin{equation}
S(\mathbf{k}, t) = \widetilde{\phi}(\mathbf{k}, t)\widetilde{\phi}(\mathbf{-k}, t)
\end{equation}

\(\mathbf{k}\) represents the wave vector in each direction of each
lattice grid point. The fluctuations in the order parameter are
calculated from the order parameter of the bijel defined as
\(\widetilde{\phi} = \phi - \langle \phi \rangle\). \(\phi\) is
calculated through subtraction of the red from the blue fluid density,
\(\phi = \rho^b - \rho^r\). The average domain size is commonly
calculated using the spherically averaged structure factor where all
points corresponding to \(k = |\mathbf{k}|\) are binned, where \(k\) is
the magnitude of \(\mathbf{k}\).

\begin{equation}
S(k, t) = \frac{1}{n} \sum_{k - \Delta < k < k + \Delta} S(\mathbf{k}, t)
\label{eq:structure_factor_calc}
\end{equation}

The average domain size can then be calculated from the spherically
averaged structure factor after calculating the first moment of the
structure factor using the following expression.

\begin{equation}
L(t) = 2\pi \frac{\sum S(k, t) }{ \sum k S(k, t) }
\end{equation}

We found that this method for determining domain size generally
overestimated the observed channel diameter. However, previous studies
have demonstrated that the scaling behavior of the first-moment-derived
domain size aligns with the expected phase separation in spinodal
decomposition. The overestimation arises due to the limited number of
samples at small k values, where the bin size is roughly equivalent to
the k value. Additionally, in simulations with periodic domains, both
the structure factor and computed length scale are influenced by
finite-size effects. Nonetheless, this measurement still provides
valuable insight into trends in the average domain size.
...
parameter. \cite{cuesta_monte_1990} First, the average orientational
tensor is calculated from the particle orientations in Eq
\ref{eq:orientational_order_tensor}

\begin{equation}
Q = \frac{1}{n_p} \sum_{i} (\frac{3}{2}[\hat{\mathbf{o}}_i \otimes \hat{\mathbf{o}}_i] - \frac{1}{2}\mathbf{I})
\label{eq:orientational_order_tensor}
\end{equation}

In Eq \ref{eq:orientational_order_tensor}, \(\hat{\mathbf{o}}_i\)
defines the orientation vector of particle \(i\) and \(\mathbf{I}\) is
the identity matrix. The nematic director or average particle
orientation is the eigenvector corresponding to the largest eigenvalue
of \(Q\) which is the value of the nematic order parameter, \(S\). The
assembly of particles can be said to have nematic order if
\(S \geq 0.3\) and is fully aligned to the director if \(S = 1.0\).
\cite{cuesta_monte_1990}
...
averaging. \cite{jansen_bijels_2011} Following the structure factor
calculation described in Eq. \ref{eq:structure_factor_calc}, the second
moment of the structure factor in direction \(\beta\) is determined for
each Cartesian coordinate axis as

\begin{equation}
L_{\beta}(t) = 2 \pi \sqrt{\frac{\sum_{\mathbf{k_{\beta}}}  S(\mathbf{k}, t) }{\sum_{\mathbf{k_{\beta}}}  k_{\beta}^2S(\mathbf{k}, t)}}
\label{eq:second_moment_domain}
\end{equation}

From Eq \ref{eq:second_moment_domain}, we can calculate the length scale
in each cartesian direction, \(L_x, L_y, L_z\). Here, we make a further
...
\(L_{\perp} = \frac{L_x + L_y}{2}\). Tortuosity has been defined using
...
effective diffusion of a tracer in the material.

To calculate the tortuosities of the structures generated from our
simulations, we first calculate \(\phi\) of the system over time from
the filled densities. Many bijels are synthesized from monomer diffused
into the oil phase, or the oil phase itself is intrinsically
polymerizable. Our next step is to binarize the order parameter with the
condition, \(\phi \leq 0 = 0\) and \(\phi > 0 = 1\) which reflects a
solid red fluid phase and void blue fluid. We then mask all unconnected
components, using the \texttt{label} function in \texttt{skimage}
leaving only the percolating domain representing the bijel.
\cite{van2014scikit} We utilize the python package taufactor which
solves the diffusion equation for the structure, calculating the time
taken for a tracer to go from one side of the system to the other.
\cite{kench_taufactor_2023} We utilize the periodic boundary condition
solver for this investigation. In previous work, we identified that the
length scale derived from the second moment of the structure factor is
only useful for identifying length scale anisotropy. Therefore, we plot
...
investigated using the average interfacial angle, \(\psi\).

The interface angle of each particle is defined as
\(\psi = \arccos{\frac{ \hat{\mathbf{o_i}}\cdot \hat{\mathbf{n_i}}}{|\hat{\mathbf{o_i}}| |\hat{\mathbf{n_i}}|}}\)
where \(\hat{\mathbf{n_i}}\) is the normal of the interface closest to
the center of the particle \(i\) and \(\hat{\mathbf{o_i}}\) is the
orientation of particle \(i\) which follows the symmetry axis of the
ellipsoid. For prolate particles, they prefer to lie flat on the
interface, meaning that with no field \(\psi = 90^{\circ}\).
\(\hat{\mathbf{n_i}}\) is calculated by first using the particle filling
algorithm described and calculating the order parameter. Next, a mesh of
the isocontour at \(\phi = 0\) is generated using the marching cubes
algorithm in scikit-image with a box length of 4. \cite{van2014scikit}
Next, the vertex closest to the center of particle \(i\) was found.
\(\hat{\mathbf{n_i}}\) was then obtained from the normal corresponding
to the vertex identified in the previous step.

Another factor affected by the application of the magnetic field is the
Steinhardt 6 fold order parameter, \(Q_6\), which characterized the
local particle ordering. \cite{steinhardt_bond-orientational_1983} It
has been used to characterize crystallization, glass transitions and
crystal structures for colloidal and molecular crystals. We utilize the
average across all particles on the interface, \(\langle Q_6 \rangle\)
and track this parameter as a function of time. Past work has identified
that local crystallization of colloidal crystals takes place at
\(\langle Q_6 \rangle = 0.38\), and that
\(\langle Q_6 \rangle^{HCP} = 0.485\).
\cite{steinhardt_bond-orientational_1983} \cite{toxvaerd_role_2020}
\cite{mickel_shortcomings_2013} We utilize a voronoi cell derived
definition of the neighbor list of each particle, addressing some of the
shortcomings highlighted in the original distance based definition.
\cite{steinhardt_bond-orientational_1983}
\cite{mickel_shortcomings_2013} We first define a complex orientational
vector, \(q_{lm}\)
...
Where \(N_b\) is the number of nearest neighbors of particle \(i\),
\(l\) is controls to the degree of spherical harmonic to be calculated,
\(m\) is an integer defined as \(-l \leq m \leq l\) that defines which
spherical harmonic is being calculated and \(\vec{r}_{ij}\) is the
distance between two particle centers. The neighbor list \(N_b\) is
usually defined by a cutoff distance from the center of particle \(i\)
to its nearest neighbors \(j\). Steinhardt then defined a bond order
parameter \(q_l\) from \(q_{lm}\) as
...
\begin{figure} 
\centering 
\includegraphics[scale=0.35]{../figures/results/paper2/domain_size-field_on-scaled.png} 
...
\begin{figure} 
\centering 
\includegraphics[scale=0.5]{../figures/results/paper2/interface_angle-nint-field_up.png} 
...
\begin{figure} 
\centering 
\includegraphics[scale=0.5]{../figures/results/paper2/interface_angle-nint-field_down.png} 
